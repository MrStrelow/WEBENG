...
/**
 * This doesn't do anything particularly useful - it just counts the total number of bytes in a request body while demonstrating how to perform non-blocking reads.
 */
public class ByteCounter extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        // a single object.
        @SuppressWarnings("unused") CounterListener listener = new CounterListener(ac, req.getInputStream(), resp.getOutputStream());
    }


    /**
     * Keep in mind that each call may well be on a different thread to the previous call. Ensure that changes in values will be visible across threads. There should only ever be one container thread at a
     * time calling the listener.
     */
    private static class CounterListener implements ReadListener, WriteListener {
        ...
        private CounterListener(AsyncContext ac, ServletInputStream sis, ServletOutputStream sos) {
            this.ac = ac;
            this.sis = sis;
            this.sos = sos;
