...
/**
 * Implementation of <code>javax.servlet.Filter</code> used to compress the ServletResponse if it is bigger than a threshold.
 *
 * @author Amy Roh
...
public class CompressionFilter implements Filter {
    ...
    public void init(FilterConfig filterConfig) {
        ...
        if (filterConfig != null) {
            ...
            if (str != null) {
                ...
                if (debug > 0) {
                    System.out.println("compressionMimeTypes set to " + Arrays.toString(compressionMimeTypes));
                }
            }
        }
    ...
    /**
     * The <code>doFilter</code> method of the Filter is called by the container each time a request/response pair is passed through the chain due to a client request for a resource at the end of the
     * chain. The FilterChain passed into this method allows the Filter to pass on the request and response to the next entity in the chain.<p> This method first examines the request to check whether the
     * client support compression. <br> It simply just pass the request and response if there is no support for compression.<br> If the compression support is available, it creates a
     * CompressionServletResponseWrapper object which compresses the content and modifies the header if the content length is big enough. It then invokes the next entity in the chain using the FilterChain
