...
public class JSONQuestionDataProvider implements QuestionDataProvider {
    ...
    private Comparator<Question> questionSorter = new Comparator<Question>() {
        ...
        public int compare(Question q1, Question q2) {
            if (q1 == null || q2 == null) throw new IllegalArgumentException("Questions can not be null.");
    ...
    public List<Category> getCategoryData() {
        ...
        List<Category> categories = gson.fromJson(new InputStreamReader(inputStream, Charsets.UTF_8), collectionType);
    ...
    protected class QuestionDeserialzier implements JsonDeserializer<Question> {
        ...
        public Question deserialize(JsonElement json, Type type, JsonDeserializationContext context) throws JsonParseException {
            ...
            for (JsonElement wrongAnswer : object.get(MEMBER_WRONG_ANSWERS).getAsJsonArray()) {
                SimpleAnswer answer = context.deserialize(wrongAnswer, new TypeToken<SimpleAnswer>() {
                }.getType());
            ...
            for (JsonElement correctChoice : object.get(MEMBER_RIGHT_ANSWERS).getAsJsonArray()) {
                SimpleAnswer answer = context.deserialize(correctChoice, new TypeToken<SimpleAnswer>() {
                }.getType());
                answer.setId(lastChoiceId++);
